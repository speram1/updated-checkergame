...
public static boolean canCapture(String path, String who, Piece[][] board,int row, int column, boolean isPlayer)
{
...
public static boolean canCaptureLeft(String who, Piece[][] board,int row, int column, boolean isPlayer)
{
	...
	if(Main.inBounds(row+rowAlter, column+colAlter) && board[row+rowAlter][column+colAlter]!= null 
			&& !board[row+rowAlter][column+colAlter].getPlace().equals("X") 
	...
	if(victimExists && safeLand && Arrays.asList(pieces).contains(board[row+rowAlter][column+colAlter].getPlace()))
	{
		return true;
	}
	else
...
public static boolean canCaptureRight(String who, Piece[][] board,int row, int column, boolean isPlayer)
{
	String[] pieces = new String[6];
	int rowAlter;
	int colAlter;
...
public static void capture(String path, String who, Piece[][] board, int pieceRow,int pieceCol, boolean isPlayer)
{
...
public static Piece[][] captureLeft(String who, Piece[][] board, int pieceRow,int pieceCol, boolean isPlayer)
{
	...
	// kill victim
	board[pieceRow+rowAlter][pieceCol+colAlter].setPlace("X");
	// change killer position
	board[pieceRow+(2*rowAlter)][pieceCol+(colAlter*2)].setPlace(board[pieceRow][pieceCol].getPlace());
	board[pieceRow][pieceCol].setPlace("X");
	return board;
}

public static Piece[][] captureRight(String who, Piece[][] board, int pieceRow, int pieceCol, boolean isPlayer) 
{
	int rowAlter;		// to specify spot to be used.
	int colAlter;
